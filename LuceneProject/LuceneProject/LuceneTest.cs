...
using Lucene.Net.Analysis.PanGu;//分词器对象
using Lucene.Net.Documents;//Document对象
using Lucene.Net.Index;  //Index对象
using Lucene.Net.QueryParsers;//QueryParsers对象
using Lucene.Net.Search;//Search对象
using Lucene.Net.Store;//Store对象
...
namespace LuceneProject
{
    class LuceneTest
    {
        ...
        {
            FSDirectory directory = FSDirectory.Open(TestIndexPath);

            IndexSearcher searcher = new IndexSearcher(directory);//查找器
            {
                TermQuery query = new TermQuery(new Term("title", "英文"));//查找分词库里为英文的数据
                TopDocs docs = searcher.Search(query,null,10000);//找到的数据          query是从索引文件查找数据 ，而第二个参数Filter是从query数据中进行过滤，第三个参数表示查多少条出来
            ...
            QueryParser queryParser = new QueryParser(Lucene.Net.Util.Version.LUCENE_30,"title",new PanGuAnalyzer());//Util对象
            {
                ...
                {
                    NumericRangeFilter<int> timeFilter = NumericRangeFilter.NewIntRange("time", 20150000, 20150099, true, true);
        ...
        public static void InitIndex()
        {
            ...
            using (IndexWriter writer = new IndexWriter(directory, new PanGuAnalyzer(), true, IndexWriter.MaxFieldLength.LIMITED))//索引写入器
            {
                ...
                {
                    Document doc = new Document();//一个对象表示一条数据
                    //加入一个字段
                    doc.Add(new Field("id", i.ToString(), Field.Store.NO, Field.Index.NOT_ANALYZED));  //列名  值   是否保存值  是否分词
                    ...
                    writer.AddDocument(doc);//将Document对象写入文件(磁盘)
                }
